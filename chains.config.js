...
const chains = [
  {
    ...
    providers: [
      {
        name: 'Pontus-X (Default)',
        url: 'https://provider.dev.pontus-x.eu',
        address: '0x68C24FA5b2319C81b34f248d1f928601D2E5246B'
      },
      {
        name: 'AgroSpai (UdL)',
        url: 'https://provider.agrospai.udl.cat',
        address: '0x94549951623DD6c3265DBbB1b032d6cF48Ba7811'
      }
    ],
    ...
    providers: [
      {
        ...
        address: '0x9546d39CE3E48BC942f0be4AA9652cBe0Aff3592'
      },
      {
        name: 'AgroSpai (UdL)',
        url: 'https://provider.agrospai.udl.cat',
...
const getDefaultChainIds = () => {
  return [
    ...new Set(chains.filter((chain) => chain.isDefault).map((c) => c.chainId))
  ]
}

const getSupportedChainIds = () => {
  return [...new Set(chains.map((c) => c.chainId))]
}

const getCustomChainIds = () => {
  return [...new Set(chains.filter((c) => c.isCustom).map((c) => c.chainId))]
}

const getProvidersForChain = (chainId) => {
